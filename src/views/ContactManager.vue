        ...
        <form action="">
          <div class="row">
            <div class="col-md-6">
              <div class="row">
                <div class="col">
                  <input type="text" class="form-control" placeholder="Rechercher nom" >
                </div>
                <div class="col">
                  <input type="submit" class="btn btn-outline-dark" value="Rechercher">
                </div>
              </div>
            </div>
          </div>
    ...
    methods: {
      clickDeleteContact: async function (contactId){
        try {
          ...
          if (response) {
            let response = await ContactService.getAllContacts(); //Obtenir les nouvelle donnees
            this.contacts = response.data;
            this.loading = false;
          }
        }
